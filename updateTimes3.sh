...
#startDate="06 JAN 2017 21:59:55" MUST START ON A MONTHLY BACKUP (the first friday of the month)!!!
dasDates=()
dasCopyJobDates=()
...
#   vagrant ssh -c "date && sudo date -s \"${dasDates[$i]}\" && date && sudo systemctl restart bareos-dir && sudo systemctl restart bareos-sd && echo \"disable schedule=WeeklyCycle\" | bconsole" bareOSdirector
   ...
   vagrant ssh -c "date && sudo date -s \"${dasDates[$i]}\" && date && sudo systemctl restart bareos-dir && sudo systemctl restart bareos-sd" bareOSdirector
   log_warning "1.4 Waiting 15 seconds for backup to occur."
   sleep 600s # Wait 15 seconds for the backup to occur.
