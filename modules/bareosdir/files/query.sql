...
#21
:List Useful Backup Information:
SELECT Job.JobId AS JobID, Job.Name AS JobName, VolumeName, Job.Level, Job.JobStatus, WEEK(Job.StartTime) AS WeekNum, DAYNAME(Job.StartTime) AS DayOfWeek, Job.StartTime AS StartTime, JobFiles AS Files,ROUND(JobBytes/1024.0/1024.0/1024.0,3) AS GB FROM Job,JobMedia,Media WHERE JobMedia.JobId=Job.JobId AND JobMedia.MediaId=Media.MediaId GROUP by Job.JobID, Job.Name, VolumeName, Job.Level, Job.JobStatus, WEEK(Job.StartTime), DAYNAME(Job.StartTime), Job.StartTime, JobBytes, JobFiles ORDER by JobId, JobName, StartTime;

$22
:List date range between VolumeDurationEnd and VolRecycleTime:
select mediaid, VolumeName, FirstWritten, DATE_ADD(FirstWritten, INTERVAL VolUseDuration second) AS VolUseDurEnd, timestampdiff(DAY, FirstWritten, DATE_ADD(FirstWritten, INTERVAL VolUseDuration second)) AS DaysInVolumeUseDuration, IF(STRCMP(VolStatus, 'Used'), 'NA', DATE_ADD(LastWritten, INTERVAL Media.VolRetention second)) AS RecycleDate, VolStatus FROM Media;
